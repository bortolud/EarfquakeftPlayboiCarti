...
//    private QueryUtils(String JSONin) {
//        SAMPLE_JSON_RESPONSE = JSONin;
//    }
    ...
    public static List<Earthquake> extractEarthquakes(String SAMPLE_JSON_RESPONSE) {
        ...
        try {
            ...
            for (int i=0; i<features.length();i++) {
                ...
                //format magnitude so it looks pretty
                DecimalFormat formatter = new DecimalFormat("0.0");
                String magnitude = formatter.format(mag);
                //
                String location = properties.getString("place");
                //break up location string for aesthetics
                String LOCATION_SEPARATOR = " of ";
                String km;
                String loc;
                if (location.contains(LOCATION_SEPARATOR)) {
                    String[] parts = location.split(LOCATION_SEPARATOR);
                    km = parts[0] + LOCATION_SEPARATOR;
                    loc = parts[1];
                } else {
                    km = "Near the";
                    loc = location;
...
//                String km = parts[0] + "of"; // 52km SE of
//                String loc = parts[1]; // Chicago, Illinois
                ...
                String dateToDisplay2 = dateFormatter2.format(timeObject);

                String url = properties.getString("url");

                earthquakes.add(new Earthquake(magnitude,km,loc,dateToDisplay1,dateToDisplay2,url));
            }
